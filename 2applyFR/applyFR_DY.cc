...
void PPPF_selection(vector<Muon> pBox, vector<Muon> fBox, double *mass, double *FR, bool *evt_selection, vector<Muon> *muons) {
      muons->clear();
      Muon mu[4]; 
      for(int j=0; j<pBox.size(); j++) {
        mu[0] = pBox[j];
...
void PPFF_selection(vector<Muon> pBox, vector<Muon> fBox,  double *mass, double *FR, bool *evt_selection, vector<Muon> *muons) {
  muons->clear();
  Muon mu[4];
  for(int j=0; j<pBox.size(); j++) {
    mu[0] = pBox[j];
...
void PFFF_selection(vector<Muon> pBox, vector<Muon> fBox, double *mass, double *FR, bool *evt_selection, vector<Muon> *muons) {
  muons->clear();
  Muon mu[4];
  for(int j=0; j<pBox.size(); j++) {
    mu[0] = pBox[j];
...
void FFFF_selection(vector<Muon> pBox, vector<Muon> fBox, double *mass, double *FR, bool *evt_selection, vector<Muon> *muons) {
  muons->clear();
  Muon mu[4];
  ...
  if(dataSet == "dataB") {
    ...
    *add_index = 1;
    ...
    //str_chain = "dcap://cluster142.knu.ac.kr//pnfs/knu.ac.kr/data/cms/store/user/joon/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8/crab_DYntuple_v2p0_DYLL_M50toInf/171230_124754/0000/ntuple_skim_*"+str+".root"; // DY
		str_chain = "dcap://cluster142.knu.ac.kr//pnfs/knu.ac.kr/data/cms/store/user/joon/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8/crab_DYntuple_Run05_ExtendedGen_DYLL_M50toInf/180401_173356/0000/ntuple_skim_*"+str+".root";
    *add_index = 10+input;
  }
    ...
    *add_index = 1000+input;
		...
		if( input == 0 )
			str_chain = "dcap://cluster142.knu.ac.kr//pnfs/knu.ac.kr/data/cms/store/user/joon/DYJetsToLL_M-200to400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8/crab_DYntuple_Run05_ExtendedGen_DYLL_M200to400/180404_021037/0000/ntuple_skim_*.root";
		else
			str_chain = "dcap://cluster142.knu.ac.kr//pnfs/knu.ac.kr/data/cms/store/user/joon/DYJetsToLL_M-200to400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8/crab_DYntuple_Run05_ExtendedGen_DYLL_M200to400_2/180404_044425/0000/ntuple_skim_*.root";
    ...
    *add_index = 1000+input;
		...
		str_chain = "dcap://cluster142.knu.ac.kr//pnfs/knu.ac.kr/data/cms/store/user/joon/DYJetsToLL_M-400to500_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8/crab_DYntuple_Run05_ExtendedGen_DYLL_M400to500/180404_021125/0000/ntuple_skim_*.root";
    ...
    *add_index = 20+input;
    ...
    //str_chain = "dcap://cluster142.knu.ac.kr//pnfs/knu.ac.kr/data/cms/store/user/joon/ZGTo2LG_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8/crab_DYntuple_v20171022_EGMCorr_ZGamma/180311_151437/0000/ntuple_skim_*.root"; // TT
		str_chain = "dcap://cluster142.knu.ac.kr//pnfs/knu.ac.kr/data/cms/store/user/joon/ZGTo2LG_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8/crab_DYntuple_Run05_ExtendedGen_ZGamma/180401_173438/0000/ntuple_skim_*"+str+".root";
    *add_index = 20+input;
  }
...
void applyFR_DY(TString dataSet, int input = 0) {
  ...
  if( MuonCollection.size() < 4 ) continue;
    ...
    if( pBox.size() == 4 ) {
      ...
      if( evt_selection ) {
        ...
        if( dataSet.Index("DY") >= 0 || dataSet.Index("ZG") >= 0 ) {
          ...
          if( GenLeptonCollection.size() >= 4 ) {
						if( !DYVirtualGammaSelector(ntuple) ) continue;
      ...
      FR = 0; bFR=1;
      PPPF_selection(pBox, fBox, &mass, &FR, &evt_selection, &muons);
      if( evt_selection ) {
        if( dataSet.Index("data") < 0 )
      ...
      FR = 0; bFR=1;
      PPFF_selection(pBox, fBox, &mass, &FR, &evt_selection, &muons);
      if( evt_selection ) {
        if( dataSet.Index("data") < 0 )
          wt = wt * ( 19.72 * analyzer->EfficiencySF_EventWeight_HLT_BtoF_ID( pBox ) + 16.146 * analyzer->EfficiencySF_EventWeight_HLT_GtoH_ID( pBox ) ) / 35.866 * ( 19.72 * analyzer->EfficiencySF_EventWeight_HLT_BtoF_LooseID( fBox ) + 16.146 * analyzer->EfficiencySF_EventWeight_HLT_GtoH_LooseID( fBox ) ) / 35.866;
      ...
      FR = 0; bFR=1;
      PFFF_selection(pBox, fBox, &mass, &FR, &evt_selection, &muons);
      if( evt_selection ) {
        if( dataSet.Index("data") < 0 )
          wt = wt * ( 19.72 * analyzer->EfficiencySF_EventWeight_HLT_BtoF_ID( pBox ) + 16.146 * analyzer->EfficiencySF_EventWeight_HLT_GtoH_ID( pBox ) ) / 35.866 * ( 19.72 * analyzer->EfficiencySF_EventWeight_HLT_BtoF_LooseID( fBox ) + 16.146 * analyzer->EfficiencySF_EventWeight_HLT_GtoH_LooseID( fBox ) ) / 35.866;
      ...
      FR = 1; bFR = 1;
      FFFF_selection(pBox, fBox, &mass, &FR, &evt_selection, &muons);
      if( evt_selection ) {
        if( dataSet.Index("data") < 0 )
  ...
  TFile* f = new TFile("final_result/FR_"+dataSet+"_VirtualGamma_"+str+".root","RECREATE");
